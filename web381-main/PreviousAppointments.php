    ...
    <div class="flex flex-col bg-white px-12 pb-10">
        ...
        <div class="flex flex-row bg-white p-10 relative">
            ...
            <table class="w-full">
                ...
                <tbody>
                <?php
                        $query2="select count(*) as c from`appointment`,`pet` where PID=vpid and appointment_status='Done' ";
                        $count=mysqli_query($con,$query2);
                        $fetch = mysqli_fetch_assoc($count);
                        $pre = $fetch['c'];
                        if($pre>0)
                        {
            $sql="select * from`appointment`,`pet` where PID=vpid and appointment_status='Done' ";
            ...
            else
            {
                ...
                <span class="font-semibold text-5m app-color-gray-1">There is no previous appointment  yet</span>
      </tr>
